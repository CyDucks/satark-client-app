...
import org.cyducks.satark.auth.worker.AnonymousSignInWorker;
import org.cyducks.satark.auth.worker.EmailAuthLinkWorker;
import org.cyducks.satark.auth.worker.PhoneAuthLinkWorker;
import org.cyducks.satark.databinding.ActivityAuthBinding;
                                        ...
                                        this.currentUser.delete()
                                                .addOnCompleteListener(task ->  {
                                                    if(task.isSuccessful()) {
                                                        Log.d(TAG, "deleteAnonUser: user deleted");
                                                    }
                                                    else {
                                                        Log.d(TAG, "deleteAnonUser: cannot delete user - [" + task.getException().getMessage() + "]");
                                                    }
                                                });
                                    }
                    ...
                    WorkManager.getInstance(getApplicationContext())
                            ...
                            .observe(this, workInfo -> {
                                if(workInfo.getState().isFinished() && workInfo.getState().equals(WorkInfo.State.FAILED)) {
                                    ...
                                    if(!map.isEmpty()) {
                                        Log.d(TAG, "phoneAuthLinkFailed: output data is empty");
                                        Toast.makeText(this, workInfo.getOutputData().getString("message"), Toast.LENGTH_SHORT).show();
                                        if(this.currentUser != null) {
                                            this.currentUser.delete()
                                                    .addOnCompleteListener(task ->  {
                    ...
                    WorkManager.getInstance(getApplicationContext())
                            ...
                            .observe(this, workInfo -> {
                                if(workInfo.getState().isFinished() && workInfo.getState().equals(WorkInfo.State.FAILED)) {
                                    ...
                                    if(!map.isEmpty()) {
                                        Log.d(TAG, "emailAuthLinkFailed: output data is empty");
                                        Toast.makeText(this, workInfo.getOutputData().getString("message"), Toast.LENGTH_SHORT).show();
                                        if(this.currentUser != null) {
                                            this.currentUser.delete()
                                                    .addOnCompleteListener(task ->  {
                                    ...
                                    if(this.currentUser != null && this.currentUser.getDisplayName() == null) {
                                        ...
                                        this.currentUser.updateProfile(request)
                                                .addOnCompleteListener(task -> {
                                                    if(task.isSuccessful()) {
                                                        Log.d(TAG, "addDisplayName: name updated");
                                                    }
                                                    else {
                                                        Log.d(TAG, "addDisplayName: cannot update name - [" + task.getException().getMessage() + "]");
                                                    }
                                                });
                                    }
